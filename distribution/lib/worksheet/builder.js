...
var _addlegacyDrawingHeaderFooter = function _addlegacyDrawingHeaderFooter(promiseObj) {
    return new Promise(function (resolve, reject) {
        if (!promiseObj.ws.legacyDrawingHeaderFooter.isEmpty) {
            var dId = promiseObj.ws.relationships.indexOf('legacyDrawingHeaderFooter') + 1;
...
var sheetXML = function sheetXML(ws) {
    return new Promise(function (resolve, reject) {
        ...
        _addSheetPr(promiseObj).then(_addDimension).then(_addSheetViews).then(_addSheetFormatPr).then(_addCols).then(_addSheetData).then(_addSheetProtection).then(_addAutoFilter).then(_addMergeCells).then(_addConditionalFormatting).then(_addDataValidations).then(_addHyperlinks).then(_addPrintOptions).then(_addPageMargins).then(_addLegacyDrawing).then(_addPageSetup).then(_addPageBreaks).then(_addHeaderFooter).then(_addlegacyDrawingHeaderFooter).then(_addDrawing).then(function (promiseObj) {
            return new Promise(function (resolve, reject) {
                wsXML.end();
                resolve(xmlString);
            });
    ...
    return new Promise(function (resolve, reject) {
        ...
        ws.relationships.forEach(function (r, i) {
            ...
            if (r instanceof hyperlinks.Hyperlink) {
                ...
                relXML.ele('Relationship').att('Id', rId).att('Target', '../drawings/vmlDrawing' + ws.sheetId + '.vml').att('Type', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing');
            }
        });
