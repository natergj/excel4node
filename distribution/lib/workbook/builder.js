...
var addRootContentTypesXML = function addRootContentTypesXML(promiseObj) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    promiseObj.wb.sheets.forEach(function (s, i) {
      xml.ele('Override').att('PartName', '/xl/worksheets/sheet' + (i + 1) + '.xml').att('ContentType', 'application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml');

      if (s.drawingCollection.length > 0) {
        xml.ele('Override').att('PartName', '/xl/drawings/drawing' + s.sheetId + '.xml').att('ContentType', 'application/vnd.openxmlformats-officedocument.drawing+xml');
      }
      if (Object.keys(s.comments).length > 0) {
        xml.ele('Override').att('PartName', '/xl/comments' + s.sheetId + '.xml').att('ContentType', 'application/vnd.openxmlformats-officedocument.spreadsheetml.comments+xml');
      }
    });
    ...
    if (!promiseObj.wb.mediaCollection.isEmpty) {
      ...
      promiseObj.wb.sheets.forEach(function (ws) {
        ...
        if (!ws.legacyDrawingHeaderFooter.isEmpty) {
          ...
          var drawingsVML = xmlbuilder.begin().ele('xml');
          ...
          var sl = drawingsVML.ele('o:shapelayout').att('v:ext', 'edit');
          sl.ele('o:idmap').att('v:ext', 'edit').att('data', ws.sheetId);

          var st = drawingsVML.ele('v:shapetype').att('id', '_x0000_t75').att('coordsize', '21600,21600').att('o:spt', '75').att('o:preferrelative', 't').att('path', 'm@4@5l@4@11@9@11@9@5xe').att('filled', 'f').att('stroked', 'f');
          ...
          st.ele('v:path').att('o:extrusionok', 'f').att('gradientshapeok', 't').att('o:connecttype', 'rect');
          ...
          ws.legacyDrawingHeaderFooter.drawings.forEach(function (d) {
...
var writeToBuffer = function writeToBuffer(wb) {
  return new Promise(function (resolve, reject) {
    ...
    addRootContentTypesXML(promiseObj).then(addWorksheetsXML).then(addRootRelsXML).then(addWorkbookXML).then(addWorkbookRelsXML).then(addCorePropertiesXML).then(addSharedStringsXML).then(addStylesXML).then(addDrawingsXML).then(addLegacyHeaderFooterDrawingsXML).then(function () {
