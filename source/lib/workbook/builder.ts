...
const addRootContentTypesXML = (promiseObj) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const xmlString = xml.doc().end(promiseObj.xmlOutVars);
    promiseObj.xlsx.file('[Content_Types].xml', xmlString);
    resolve(promiseObj);
  });
};

const addRootRelsXML = (promiseObj) => {
  // Required as stated in §12.2
  return new Promise((resolve, reject) => {
    const xml = xmlbuilder
...
const addWorkBookXML = (promiseObj) => {
  // Required as stated in §12.2
  return new Promise((resolve, reject) => {
...
const addWorkBookRelsXML = (promiseObj) => {
  // Required as stated in §12.2
  return new Promise((resolve, reject) => {
    const xml = xmlbuilder
      .create('Relationships', {
...
const addWorkSheetsXML = (promiseObj) => {
  // Required as stated in §12.2
  return new Promise((resolve, reject) => {
    let curSheet = 0;

    const processNextSheet = () => {
      const thisSheet = promiseObj.wb.sheets[curSheet];
      if (thisSheet) {
        curSheet += 1;
        thisSheet
          .generateXML()
 ...
 */
const addSharedStringsXML = (promiseObj) => {
    ...
    if (!promiseObj.wb.mediaCollection.isEmpty) {
      promiseObj.wb.sheets.forEach((ws) => {
        if (!ws.drawingCollection.isEmpty) {
          ...
          const drawingsXML = xmlbuilder.create('xdr:wsDr', {
            version: '1.0',
            encoding: 'UTF-8',
            standalone: true,
          });
          ...
          ws.drawingCollection.drawings.forEach((d) => {
            if (d.kind === 'image') {
              const target = 'image' + d.id + '.' + d.extension;

              const image = d.imagePath
                ? fs.readFileSync(d.imagePath)
                : d.image;
              promiseObj.xlsx
                .folder('xl')
                .folder('media')
          ...
          const drawingsXMLStr = drawingsXML.doc().end(promiseObj.xmlOutVars);
          const drawingRelXMLStr = drawingRelXML
            .doc()
            .end(promiseObj.xmlOutVars);
          promiseObj.xlsx
            .folder('xl')
            .folder('drawings')
