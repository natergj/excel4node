var cell = function(row, col, wb){
	...
	this.String = function(val){
		if(wb.sharedStrings.indexOf(val) == -1){
			wb.sharedStrings.push(val);
		}
		wb.strings.sst[0]['@count']=wb.strings.sst[0]['@count']+1;
		this['@t'] = 's'
		this.v = wb.sharedStrings.indexOf(val);
		return that;
	}
...
exports.Row = function(row1){
	...
	if(this.sheet.sheetData.length < row1){
		for(var i = this.sheet.sheetData.length; i < row1; i++){
			this.sheet.sheetData.push({row:[{'@r':row1}]});
		}
	}
	...
	this.addCell = function(col){
		if(this.cellCount() < col){
			for(var i = this.cellCount(); i <= col; i++){
				thisCell = new cell(row1, i + 1, that.wb.workbook);
