var Column = function(colNum){
	...
	if(!thisWS.cols[colNum]){
		thisWS.cols[colNum] = new column();
	}

	var thisCol = thisWS.cols[colNum];
	thisCol.ws = thisWS;
	thisCol.setAttribute('max',colNum);
	thisCol.setAttribute('min',colNum);
	thisCol.setAttribute('customWidth',0);
	thisCol.setAttribute('width',thisWS.wb.defaults.colWidth);
