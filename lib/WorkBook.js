...
exports.WorkBook = function(){
	...
	this.write = function(fileName, response){
		...
		if(this.workbook.drawings){
			var drawingRelsXML = xml.create(this.workbook.drawings.rels);
			console.log(drawingRelsXML.end({pretty:true, indent:'  ', newline: '\n'}));

			this.workbook.drawings.drawings.forEach(function(d){
				var drawingXML = xml.create(d.xml);
				console.log(drawingXML.end({pretty:true, indent:'  ', newline:'\n'}));
			});
		}
