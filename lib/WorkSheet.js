		...
		*/
		...
		Object.keys(thisWS.rows).forEach(function(r,i){
			...
			Object.keys(thisWS.rows[r].attributes).forEach(function(a,i){
				thisRow.row['@'+a] = thisWS.rows[r].attributes[a];
			});
			var cells = thisRow.row.c = [];
			Object.keys(thisWS.rows[r].cells).forEach(function(c,i){
				var thisCellIndex = cells.push({});
				var thisCell = cells[thisCellIndex - 1];
		...
		var wsXML = xml.create({'worksheet':thisWS.sheet});
