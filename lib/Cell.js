...
function excelRefSort(a, b){
	...
	if(a.replace(/[0-9]/g,'') === b.replace(/[0-9]/g,'')){
		...
		if(alphaOne.length != alphaTwo.length){
			return alphaOne.length - alphaTwo.length;
		}else{
			for(var i = 0; i < alphaOne.length; i++){
				if(alphaOne.substr(i,1) != alphaTwo.substr(i,1)){
					return alphaOne.charCodeAt(i) - alphaTwo.charCodeAt(i);
				}
			}
			return a.replace(/[a-zA-Z]/g,'') - b.replace(/[a-zA-Z]/g,'');
		}
...
function excelCellsSort(a, b){
	...
	if(ar.replace(/[0-9]/g,'') === br.replace(/[0-9]/g,'')){
		...
		var alphaOne = ar.replace(/[0-9]/g,'').toUpperCase();
		var alphaTwo = br.replace(/[0-9]/g,'').toUpperCase();
		if(alphaOne.length != alphaTwo.length){
			return alphaOne.length - alphaTwo.length;
		}else{
			for(var i = 0; i < alphaOne.length; i++){
