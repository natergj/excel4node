...
var cell = function(ws){
	...
	thisCell.deleteAttribute = deleteAttribute;
	thisCell.getStyleInfo = getStyleInfo;
	thisCell.toCellRange = getCellRange;
	...
	function deleteAttribute(attr){
		return delete thisCell.attributes[attr];
	}
	function getStyleInfo(){
		var styleData = {};
		if(thisCell.getAttribute('s')){
			var wbStyleData = thisCell.ws.wb.styleData;
			var xf = wbStyleData.cellXfs[thisCell.getAttribute('s')];
			styleData.xf = xf;
	...
	function getCellRange(){
		...
		var rc = thisCell.getAttribute('r').toExcelRowCol();
		return thisCell.ws.Cell(rc.row,rc.col);
	}
