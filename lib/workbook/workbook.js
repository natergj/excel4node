...
function WorkBook(opts) {
    ...
    if (opts.allowInterrupt) {
        Array.prototype.e4nForEach = utils.aSyncForEach;
    } else {
        Array.prototype.e4nForEach = Array.prototype.forEach;
    }
    ...
    if (opts.jszip) {
        Object.keys(opts.jszip).e4nForEach(function (k) {
...
WorkBook.prototype.createStyleSheetXML = function () {
    ...
    items.e4nForEach(function (i) {
        data.styleSheet[i].push({ '@count': thisWB.styleData[i].length });
        thisWB.styleData[i].e4nForEach(function (d) {
            data.styleSheet[i].push(d.generateXMLObj());
        });
    });

    this.dxfCollection.getBuilderElements().forEach(function (builderElement) {
        data.styleSheet['dxfs'] = data.styleSheet['dxfs'] || [];
        data.styleSheet['dxfs'].push(builderElement);
    });
...
WorkBook.prototype.writeToBuffer = function () {
    ...
    this.worksheets.e4nForEach(function (sheet, i) {
        var sheetCount = i + 1;
        var thisRId = utils.generateRId();
        var sheetExists = false;

        wbOut.workbook.sheets.e4nForEach(function (s) {
        ...
        if (sheet.drawings) {
            ...
            sheet.drawings.drawings.e4nForEach(function (d) {
        ...
        if (sheet.hyperlinks) {
            ...
            sheet.hyperlinks.e4nForEach(function (cr, i) {
                var found = false;
                sheet.rels.Relationships.e4nForEach(function (r) {
    ...
    wbOut.sharedStrings.e4nForEach(function (s) {
    ...
    // TODO variables should not be deleted
    delete wbOut;
