...
var style = function(wb,opts){
	...
	curStyle.wb = wb;
	curStyle.xf = new cellXfs(wb,opts);
	...
	function numberFunctions(){
		...
		function setFormat(fmt){
			if(!wb.styleData.numFmts[curStyle.xf.numFmtId - 164]){
				var curFmt = new numFmt(curStyle.wb);
				console.log(curFmt);
				console.log(wb.styleData.numFmts);
			}else{
				var curFmt = JSON.parse(JSON.stringify(wb.styleData.numFmts[curStyle.xf.numFmtId - 164]));
			}
			curFmt.formatCode = fmt;
			var thisFmt = new numFmt(curStyle.wb, curFmt);
			console.log(thisFmt);
			var curXF = curStyle.xf;
			curXF.numFmtId = thisFmt.numFmtId;
			curStyle.xf = new cellXfs(curStyle.wb, curXF);
		}
	...
	function fillFunctions(){
		...
		function setFillColor(color){
			curFill.fgColor = cleanColor(color);
			curFill.bgColor = cleanColor(color);
			var thisFill = new fill(curStyle.wb,curFill);
			var curXF = curStyle.xf;
			curXF.applyFill=1;
	...
	function fontFunctions(){
		...
		function setTextWrap(){
			if(!curStyle.xf.alignment){
				curStyle.xf.alignment = {};
			}
			curStyle.xf.applyAlignment=1;
			curStyle.xf.alignment.wrapText=1;
		}
		function setFontAlignmentVertical(val){
			...
			curStyle.xf.applyAlignment=1;
			curStyle.xf.alignment.vertical = val;
		}
		function setFontAlignmentHorizontal(val){
			...
			curStyle.xf.applyAlignment=1;
			curStyle.xf.alignment.horizontal = val;
		}
		return this;
	}

	function setBorder(opts){
		var curBorder = new border(curStyle.wb,opts);
		var curXF = curStyle.xf;
		curXF.applyBorder=1;
...
var cellXfs = function(wb,opts){
	...
	if(wb.styleData.cellXfs.length == 0){
		...
		/*
		<xf applyAlignment="1" applyBorder="1" applyFill="1" applyFont="1" applyNumberFormat="0" applyProtection="0" borderId="9" fillId="2" fontId="2" numFmtId="0">
		...
		ordinals.forEach(function(o){
			if(curBorder[o]){
				...
				tmpObj[o] = [
					...
					{
						'color':{
							'@rgb':curBorder[o].color?cleanColor(curBorder[o].color):'FFFFFFFF'
						}
					}
				];
...
function cleanColor(val){
	...
	if(Object.keys(excelColors).indexOf(val.toLowerCase()) >= 0){
	...
	}else if(val.length == 7 && val.substr(0,2) == '#' && /^[a-fA-F0-9()]+$/.test(val)){
		// val was sent as html style hex code, remove # and add alpha
		return 'FF'+val.substr(1).toUpperCase();
	}else if(val.length == 9 && val.substr(0,2) == '#' && /^[a-fA-F0-9()]+$/.test(val.substr(1))){
		...
		console.log('%s is an invalid color option. chnaging to white',val);
