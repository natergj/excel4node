exports.Style = function(){
	...
	this.getStyleId = function(){
		return that.data.IDs.xf;
	}

	this.Clone = function(){
		var clonedStyle = that.wb.Style();
		clonedStyle.data = deepClone(that.data);
		return clonedStyle;
	};
	...
	this.setXF = function(){
		...
		that.wb.styles['@'+curCount] = that;
		that.data.IDs.xf = curCount;
	}
	this.setNumFmtID = function(fmtId){
		...
		//if(that.data.IDs.numFmt == 0){
			...
			that.data.IDs.numFmt = fmtId?fmtId:curCount;
		//}else{
	...
	this.Border = function(obj){
		...
		*/
		...
		/*
			takes input border definition object and replaces ordinal border definitions
		*/
		...
		that.wb.workbook.styles.styleSheet.borders.forEach(function(b){
			if(b['@count']){
				b['@count'] += 1;
			}
		});
...
function cleanColor(rgb){
	if(rgb.length == 6){
		rgb='FF'+rgb;
	}else if(rgb.substring(0,1) == '#'){
		rgb='FF'+rgb.substring(1)
	}else if(rgb.length==8){
		rgb=rgb;
	}else{
		console.log('invalid fill color value');
	}
