...
function WorkSheet(wb) {
    ...
    this.conditionalFormattingRulesBySqref = {};
...
WorkSheet.prototype.toXML = function () {
    ...
    EXCEL_XML_TAG_ORDER.e4nForEach(function (k) {
        ...
        if (k === 'conditionalFormatting') {
            // conditional formatting special case uses multiple tags
            var cfTags = getConditionalFormattingTagData(thisWS);
            cfTags.forEach(function (t) {
                wsXML.ele({ 'conditionalFormatting': t });
            });
        }
        if (thisSheet[k]) {
            var def = {};
...
WorkSheet.prototype.addConditionalFormattingRule = function (sqref, ruleProps) {
    var rules = this.conditionalFormattingRulesBySqref[sqref] || [];
    rules.push(ruleProps);
    this.conditionalFormattingRulesBySqref[sqref] = rules;
    return this;
};
...
function processConditionalFormatting(thisWS, thisSheet) {
    var dict = thisWS.conditionalFormattingRulesBySqref;
    if (!Object.keys(dict).length) {
        return;
    }
    thisSheet['conditionalFormatting'] = thisSheet['conditionalFormatting'] || [];
    Object.keys(dict).e4nForEach(function (sqref) {
        var rules = dict[sqref];
        thisSheet['conditionalFormatting'].push({
            '@sqref': sqref,
            rules: rules
        });
    });
...
function getConditionalFormattingTagData(thisWS) {
    var data = [];
    var dict = thisWS.conditionalFormattingRulesBySqref;
    if (!Object.keys(dict).length) {
        return data;
    ...
    Object.keys(dict).e4nForEach(function (sqref) {
        ...
        data.push({
            '@sqref': sqref,
            cfRule: rules.map(function (r, idx) {
                return {
                    '@type': r.type,
                    '@dxfId': idx, // TODO what is dxfId?
                    '@priority': r.priority,
                    '@operator': r.operator,
                    '@text': r.text,
                    formula: r.formula
