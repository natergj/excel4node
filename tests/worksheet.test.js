...
test('WorkSheet test paperWidth/paperHeight', function (t) {
	t.plan(2);
	var wb = new xl.WorkBook();
	var ws = wb.WorkSheet('test', {
		paperDimensions: {
	...
	t.equal(ws.sheet.pageSetup.length, 2, 'The pageSetup should contain 2 elements');
	t.equal(JSON.stringify(ws.sheet.pageSetup), '[{"@paperWidth":"297mm"},{"@paperHeight":"420mm"}]');
});

test('WorkSheet test paperSize', function (t) {
	t.plan(2);
	var wb = new xl.WorkBook();
	var ws = wb.WorkSheet('test', {
		paperDimensions: {
	...
	t.equal(ws.sheet.pageSetup.length, 1, 'The pageSetup should contain 1 element');
	t.equal(JSON.stringify(ws.sheet.pageSetup), '[{"@paperSize":9}]');
});
