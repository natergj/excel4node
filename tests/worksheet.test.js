...
test('WorkSheet test printScaling', function (t) {
    ...
    t.equal(JSON.stringify(ws.sheet.pageSetup), '[]', 'NO_SCALING') ;

    opts.scale = wb.Print.FIT_ONE_PAGE;
    ws.printScaling(JSON.parse(JSON.stringify(opts)));
    t.equal(JSON.stringify(ws.sheet.pageSetup), '[{"@horizontalDpi":12345},{"@verticalDpi":67890}]', 'FIT_ONE_PAGE') ;

    opts.scale = wb.Print.FIT_ALL_COLUMNS;
    ws.printScaling(JSON.parse(JSON.stringify(opts)));
    t.equal(JSON.stringify(ws.sheet.pageSetup), '[{"@fitToHeight":0},{"@horizontalDpi":12345},{"@verticalDpi":67890}]', 'FIT_ALL_COLUMNS') ;

    opts.scale = wb.Print.FIT_ALL_ROWS;
    ws.printScaling(JSON.parse(JSON.stringify(opts)));
    t.equal(JSON.stringify(ws.sheet.pageSetup), '[{"@fitToWidth":0},{"@horizontalDpi":12345},{"@verticalDpi":67890}]', 'FIT_ALL_ROWS') ;

    opts.scale = wb.Print.CUSTOM_SCALING;
    ws.printScaling(JSON.parse(JSON.stringify(opts)));
    t.equal(JSON.stringify(ws.sheet.pageSetup), '[{"@fitToHeight":300},{"@fitToWidth":200},{"@horizontalDpi":12345},{"@verticalDpi":67890}]', 'CUSTOM_SCALING') ;
});